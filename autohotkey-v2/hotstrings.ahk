; Script Name: hotstrings.ahk
; Description: AutoHotkey hotstrings for usually I typically misspell
; GitHub Repository: https://github.com/ifcodesdotnet/.iffiles
; References:
;   https://www.autohotkey.com/docs/v2/Hotstrings.htm
;     Link to hotstrings documentation

;   https://www.autohotkey.com/docs/v2/Hotstrings.htm#EndChars
;     Link to hotstrings documentation that explains how Ending Characters work
;
; Research: There is an issue with all of the hotstrings when trying to use them
;           in Notepad on Windows 11. It seems like I can circumvent the issue if
;           I were to write my hotstrings like this:
;   :*:maint`n::
;   {
;       Send maintenance
;   }



:*:arch`n::architecture

:*:maint`n::maintenance

:*:enging`n::engineering

:*:eng`n::engineer

:*:rel`n::relative

:*:spec`n::specific

:*:repo`n::repository

:*:repos`n::repositories

:*:deps`n::dependencies

:*:dep`n::dependency

:*:rec`n::receive

:*:sch`n::schedule

:*:dir`n::directory