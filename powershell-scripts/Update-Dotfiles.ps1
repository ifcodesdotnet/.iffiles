...
function main {
    $configurations = Get-Content -Path ".\configurations.json" -Raw | ConvertFrom-Json
    ...
    foreach($configuration in $configurations) {
        ...
        Update-Configuration -name $($configuration.'name') -target $($configuration.'target-directory')
    }
}
